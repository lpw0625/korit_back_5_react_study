...
function ImageEx() {
    ...
    // 어떠한 상태들이 있는지 확인
    // 자신이 모르는 코드가 있으면 찾아보기.
    // 
    useEffect(() => {
        setOldFiles(!localStorage.getItem("oldFiles") ? [] : JSON.parse(localStorage.getItem("oldFiles")));
    }, []);

    // 로컬호스티지에 키값이 없는 경우(비어있는 배열)를 정의를 해둠.
    //컴포넌트를 통해 어떠한 구조를 짯는지 확인.
    /// filelisht 갹채는 


    const handleFileChange = (e) => {
        console.log(e.target.files)
        const loadFiles = Array.from(e.target.files); //ㅅ
        ...
        Promise.all(promises) // >> new promise가 실행이 되는 순간 promise가 실행이 됨.
        .then(result => {
